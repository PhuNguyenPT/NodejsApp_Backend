...
morgan.token("response-time-colored", (req: Request, res: Response) => {
  const responseTimeToken = (
    morgan as unknown as {
      "response-time": (req: Request, res: Response) => string | undefined;
    }
  )["response-time"];
  const responseTimeStr = responseTimeToken(req, res) ?? "0";
  const responseTime = parseFloat(responseTimeStr);

  if (responseTime > 1000) return `\x1b[31m${responseTime.toString()}ms\x1b[0m`; // Red for > 1s
  if (responseTime > 500) return `\x1b[33m${responseTime.toString()}ms\x1b[0m`; // Yellow for > 500ms
